...
router.get('/profile', authenticateToken, asyncHandler(async (req, res) => {
  const { data: userData, error } = await dbAdmin
    .from('users')
    .select(`
      *,
      user_documents (
...
router.post('/avatar', authenticateToken, upload.single('avatar'), logUserAction('avatar_upload'), asyncHandler(async (req, res) => {
  ...
  const publicStorageBaseUrl = (process.env.PUBLIC_STORAGE_BASE_URL || '').replace(/\/$/, '');
  const avatarUrl = publicStorageBaseUrl
    ? `${publicStorageBaseUrl}/avatars/${req.user.id}/${Date.now()}-${req.file.originalname}`
    : `/avatars/${req.user.id}/${Date.now()}-${req.file.originalname}`;
...
router.post('/documents', authenticateToken, upload.fields([
  ...
  const { data: existingDoc } = await dbAdmin
    .from('user_documents')
    .select('id')
  ...
  const publicStorageBaseUrl = (process.env.PUBLIC_STORAGE_BASE_URL || '').replace(/\/$/, '');
  const documentFrontUrl = publicStorageBaseUrl
    ? `${publicStorageBaseUrl}/documents/${req.user.id}/${Date.now()}-front.jpg`
    : `/documents/${req.user.id}/${Date.now()}-front.jpg`;
  const documentBackUrl = req.files?.document_back
    ? (publicStorageBaseUrl
        ? `${publicStorageBaseUrl}/documents/${req.user.id}/${Date.now()}-back.jpg`
        : `/documents/${req.user.id}/${Date.now()}-back.jpg`)
    : null;
  const selfieUrl = req.files?.selfie
    ? (publicStorageBaseUrl
        ? `${publicStorageBaseUrl}/documents/${req.user.id}/${Date.now()}-selfie.jpg`
        : `/documents/${req.user.id}/${Date.now()}-selfie.jpg`)
    : null;

  // Insérer le document
  const { data: newDocument, error: insertError } = await dbAdmin
    .from('user_documents')
    .insert({
      user_id: req.user.id,
      document_type,
...
router.get('/documents', authenticateToken, asyncHandler(async (req, res) => {
  const { data: documents, error } = await dbAdmin
    .from('user_documents')
...
router.delete('/account', authenticateToken, logUserAction('account_deletion'), asyncHandler(async (req, res) => {
  ...
  // Révoquer toutes les sessions (refresh tokens) côté API
  await refreshTokenService.revokeAllForUser({ userId: req.user.id });
