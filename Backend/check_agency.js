...
async function run() {
  ...
  const res = await client.query(`SELECT id, user_id_display, full_name, role FROM public.users WHERE user_id_display LIKE '%753380%'`);
  if (res.rows.length === 0) {
    console.log('Utilisateur 753380 non trouve');
  } else {
    for (const u of res.rows) {
      console.log('Utilisateur trouvé:', u);
      const ag = await client.query('SELECT * FROM public.agency_memberships WHERE user_id = $1', [u.id]);
      if (ag.rows.length > 0) {
